...
export function RoomVisualizer() {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1.1fr_0.9fr]">
      <div className="relative min-h-[420px] overflow-hidden rounded-lg bg-gradient-to-br shadow-soft">
      ...
      <div>
        ...
        <div className="mt-6 grid gap-3">
          {modes.map((item) => (
            <button key={item.label} className={`rounded-lg border p-4 text-left font-semibold ${mode.label === item.label ? "border-gold-500 bg-gold-100" : "border-forest-900/10 bg-white"}`} type="button" onClick={() => setMode(item)}>
              {item.label}
            </button>
          ))}
        </div>
