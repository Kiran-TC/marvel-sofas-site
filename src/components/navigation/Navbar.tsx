...
export function Navbar({ onSearch }: { onSearch: () => void }) {
  ...
  const solidHeader = solid || !isHome;
  ...
  const linkClass = ({ isActive }: { isActive: boolean }) =>
    cn(
      ...
      solidHeader ? "text-forest-950 hover:bg-forest-900/5" : "text-white hover:bg-white/10",
      isActive && (solidHeader ? "bg-gold-100 text-forest-950" : "bg-white/15 text-gold-100"),
    );
    ...
    <header className={cn("fixed inset-x-0 top-0 z-50 transition-all duration-300", solidHeader ? "bg-ivory/90 shadow-sm backdrop-blur-xl" : "bg-transparent")}>
      <div className="luxury-shell flex h-20 items-center justify-between gap-4">
        ...
        <div className="hidden shrink-0 items-center gap-1.5 lg:flex xl:gap-2">
          <button className={cn("rounded-full p-3", solidHeader ? "text-forest-950 hover:bg-forest-900/5" : "text-white hover:bg-white/10")} type="button" onClick={onSearch} aria-label="Open search">
            <Search className="h-5 w-5" />
          </button>
          <button className={cn("rounded-full p-3", solidHeader ? "text-forest-950 hover:bg-forest-900/5" : "text-white hover:bg-white/10")} type="button" onClick={() => setDark((value) => !value)} aria-label="Toggle colour mode">
        ...
        <button
          className={cn(
            "relative z-[60] rounded-full border p-3 backdrop-blur lg:hidden",
            solidHeader ? "border-forest-900/10 bg-white/82 text-forest-950 shadow-sm" : "border-white/15 bg-forest-950/42 text-white",
          )}
          type="button"
      ...
      <AnimatePresence>
        {mobileOpen ? (
          <motion.div
            className="fixed inset-0 z-[100] h-[100dvh] overflow-y-auto bg-forest-950 text-white lg:hidden"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          >
            <div className="flex min-h-[100dvh] flex-col px-5 pb-6 pt-4">
              <div className="flex items-center justify-between gap-4 border-b border-white/10 pb-4">
                ...
                <button className="rounded-full border border-white/15 p-3 text-white hover:bg-white/10" type="button" onClick={() => setMobileOpen(false)} aria-label="Close menu">
                  <X className="h-6 w-6" />
                </button>
              </div>
              <nav className="grid gap-2 py-6" aria-label="Mobile navigation">
                {primaryNavigation.map((item) => (
                  <Link key={item.label} to={item.href} onClick={() => setMobileOpen(false)} className="rounded-lg border border-white/12 bg-white/[0.04] px-5 py-4 font-display text-2xl text-white shadow-[0_14px_40px_rgba(0,0,0,0.14)]">
                    {item.label}
                  </Link>
                ))}
              </nav>
